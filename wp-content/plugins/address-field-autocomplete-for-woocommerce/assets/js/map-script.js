jQuery( document ).ready(
	function ($) {
		jQuery( document ).on(
			...
			".search_input",
			function () {
				jQuery( 'input.find_location' ).val( "false" );
			}
		);
		...
		jQuery( '#billing_country_field' ).on(
			"change",
			function (e, args){
				jQuery( ".select2-selection" ).removeClass( "select2-selection select2-selection--single" );
			}
		);
		jQuery( '#select2-shipping_country-container' ).on(
			"change",
			function (e, args){
				jQuery( ".select2-selection" ).removeClass( "select2 select2-container select2-container--default select2-container--below" );
			}
		);
	}
);
...
function initAutocomplete(map_element, search_input, billing_shipping_address, default_country) {
	...
	if ( aafw_enable_restriction.aafw_enable_restriction.includes( "1" ) ) {
				...
				function(){
					if (value == 'administrative_area_level_1' && filtered_array.length) {
						...
						if ( jQuery( '#billing_state' ).length ) {
							...
							jQuery( '#billing_state' ).select2().trigger( 'change' ).removeClass( "select2-selection select2-selection--single" );
...
function shippingAutocomplete(default_country, billing_shipping_address) {
	let map, marker, geocoder, infoWindow;
		var s_input = document.getElementById( 'shipping_address_map_search_input' );
		 let ship_country;
	if (ship_countries.ship_countries.includes( "all" )) {
		country = [];
	} else {
		ship_country = ship_countries.ship_countries;
	}
	...
	const options     = {
		bounds: defaultBounds,
		componentRestrictions: { country: ship_country },
		fields: ["address_components", "geometry", "icon", "name"],
		strictBounds: false,
		types: ["establishment"],
	};
	...
	if ( aafw_enable_restriction.aafw_enable_restriction.includes( "1" ) ) {
		var s_autocomplete = new google.maps.places.Autocomplete( s_input, options );
	} else {
		var s_autocomplete = new google.maps.places.Autocomplete( s_input, options );
	}
