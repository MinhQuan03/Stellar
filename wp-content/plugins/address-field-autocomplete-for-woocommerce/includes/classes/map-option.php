...
if ( ! class_exists( 'Map_Option' ) ) {
	...
	class Map_Option {
		...
		public function map_option_options() {
			...
			// add setting to create option to disable map from frontend.
			add_settings_field( 'aafw_enable_restriction', __( 'Selling location', 'address-autocomplete' ), array( $this, 'selling_location' ), 'map-options', 'map_fields' );
			...
			register_setting( 'map_fields', 'aafw_enable_restriction' );
		...
		public function selling_location() {
			$aafw_enable_restriction = get_option( 'aafw_enable_restriction', 1 );
			?>
			
			<input type="checkbox" id="aafw_enable_restriction" class="aafw_enable_restriction" name="aafw_enable_restriction" value="1" <?php checked( 1, $aafw_enable_restriction, true ); ?> />
			<span> <?php esc_html_e( 'Enable this, to restrict the result of address autocomplete to specific countries. If no countries are selected result will not be restricted.' ); ?> </span>
			<?php
